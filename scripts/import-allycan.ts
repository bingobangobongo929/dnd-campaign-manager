    ...
    for (const char of allCharacters) {
      const { data: charData, error: charError } = await supabase
        ...
        .insert({
          ...
          status: (char as any).status || 'alive',
          summary: (char as any).summary || null,
          backstory: (char as any).backstory || null,
          personality: (char as any).personality || null,
          secrets: (char as any).secrets || null,
          important_people: (char as any).important_people || [],
          story_hooks: (char as any).story_hooks || [],
          position_x: posX,
          position_y: posY,
        })
        .select()
