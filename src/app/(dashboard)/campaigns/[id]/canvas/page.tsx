...
export default function CampaignCanvasPage() {
  ...
  const canvasActions = (
    <>
      ...
      {campaign && (
        <TemplateStateBadge
          mode={campaign.content_mode || 'active'}
          inactiveReason={campaign.inactive_reason}
          size="sm"
        />
      )}
  ...
  // Group characters by type (PC vs NPC) for mobile Kanban view
  const pcCharacters = characters.filter(c => c.type === 'pc')
  const npcCharacters = characters.filter(c => c.type === 'npc')

  // Build kanban columns based on type and canvas groups
  const kanbanColumns = [
    { id: 'pc', name: 'Player Characters', color: '#8B5CF6', icon: 'users', characters: pcCharacters },
