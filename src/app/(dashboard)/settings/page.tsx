            ...
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-3">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-white/[0.06]">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase tracking-wide">Discord</label>
                    {(settings as { discord_username?: string | null })?.discord_username ? (
                      <p className="text-sm text-white">
                        {(settings as { discord_username?: string | null }).discord_username}
                      </p>
                    ) : (
                ...
                {(settings as { discord_id?: string | null })?.discord_id ? (
                  <button
                    onClick={handleUnlinkDiscord}
                    disabled={discordUnlinking}
                    className="text-xs text-red-400 active:text-red-300"
                  >
                    {discordUnlinking ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Unlink'}
                  </button>
                ) : (
                  <button
                    onClick={handleLinkDiscord}
                    disabled={discordLinking}
                    className="text-xs text-[#5865F2] active:text-[#4752C4]"
                  >
                    {discordLinking ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Link'}
                  </button>
                )}
              </div>
            </div>
          ...
          <div className="card p-5 space-y-4">
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-[--bg-elevated] border border-[--border]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <label className="text-xs text-[--text-tertiary] uppercase tracking-wide">Discord</label>
                  {(settings as { discord_username?: string | null })?.discord_username ? (
                    <p className="text-[--text-primary] font-medium">
                      {(settings as { discord_username?: string | null }).discord_username}
                    </p>
